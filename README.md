...
For **Bank card**, cards are validated with the **Luhn algorithm** and brand regexes for Visa / Mastercard / Amex / Discover / Diners / JCB, and the expiration date must be current/future.

For **Bank account**, the card fields are not required. The intake instead requires Bank Name, Account Number, and a 9-digit Routing Number.
...
- Accepts either **Bank card** or **Bank account** payment paths.
- Bank template fields parsed: Bank Name, Account Number, Routing Number.
- Current pre-filled form button is labeled **Ahmed Osama form**.
- Added **Osama Medguard** destination using the MGA New Client Form.
- Medguard defaults: Dealer ID = `Bell Marketing 1`, Transfer DID = `8559993187`.
- Device translation for Medguard: Necklace → `Smart-Go` / `39.95`; Smartwatch → `Watch - CWL` / `44.95`.
- Run `setup.html` against the Medguard form once to populate `MEDGUARD_CONFIG` entry IDs in `config.js`.
